...
import { Loader2, Send, AlertCircle, MessageCircle } from 'lucide-react';
import { getPerplexityResponse } from '../lib/perplexity';
import { validateMessageLimit } from '../lib/tiers';
...
export const BotView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!message.trim() || isProcessing || !id) return;
    ...
    try {
      ...
      const errorMessage = err.message.includes('No messages remaining')
        ? err.message
    ...
    <div className="min-h-screen pt-24 pb-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-purple-600/90 to-pink-500/90 p-8 rounded-xl backdrop-blur-lg">
          ...
          <div className="mb-8 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg">
                <MessageCircle className="w-5 h-5 text-[#19FF7F]" />
                <span className="text-white font-medium">{messagesRemaining} messages remaining</span>
              </div>
            </div>
          ...
          {/* Message Input and Upgrade Button */}
          <div className="space-y-4">
            <form onSubmit={handleSubmit} className="flex gap-2">
              ...
              <button
                type="submit"
                disabled={isProcessing || messagesRemaining <= 0}
                className="px-6 py-2 bg-[#19FF7F] text-black rounded-lg hover:bg-[#19FF7F]/90 transition-colors duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isProcessing ? (
                  <>
              ...
              <div className="flex items-center justify-between p-4 bg-white/10 rounded-lg">
                <div className="text-white">
                  ...
                  <p className="text-sm text-white/70">
                    Upgrade your plan to continue the conversation
                  </p>
                </div>
                <Link
                  to="/pricing"
